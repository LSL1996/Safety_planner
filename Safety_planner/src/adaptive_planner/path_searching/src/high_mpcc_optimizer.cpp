...
void high_mpcc_optimizer::calCostFunctionandGradient(){
    ...
    for(int i = 1;i < N_; i++){
        ...
        if(abs(dist) < dist_1_){
            cout <<"env adp start" << endl;
            if(vel.norm() < 0.01 || dist_grad.norm() < 0.001){
                fv     += pow(dist-dist_1_,2) * pow(vel.norm()-0.1 ,2);
                Gradient_vx += (2*(dist-dist_1_)*dist_grad(0) * A_.row(3*i) * pow(vel.norm()-0.1,2) + 2 * pow(dist-dist_1_,2) * vel(0) * (vel.norm()-0.1) / vel.norm() * A_.row(3*i+1));
                Gradient_vy += (2*(dist-dist_1_)*dist_grad(1) * A_.row(3*i) * pow(vel.norm()-0.1,2) + 2 * pow(dist-dist_1_,2) * vel(1) * (vel.norm()-0.1) / vel.norm() * A_.row(3*i+1));
                Gradient_vz += (2*(dist-dist_1_)*dist_grad(2) * A_.row(3*i) * pow(vel.norm()-0.1,2) + 2 * pow(dist-dist_1_,2) * vel(2) * (vel.norm()-0.1) / vel.norm() * A_.row(3*i+1));
            }
            else{
                //cout <<"high_mpc_eva_start"<< endl;
                edt_map_->evaluateSecondGrad(pos, dist_grad2);
                beta = vel.dot(dist_grad) / vel.norm() / dist_grad.norm();
                f_beta = 2/(1+exp(-K_*beta));
                p_f_beta_beta = 2*K_*exp(K_*beta)/pow(1+exp(K_*beta),2);
                fv += pow(dist-dist_1_,2) * pow(vel.norm() - f_beta * vel_min_,2);
                ...
                //cout << "p_f_beta_uz \n  "<< p_f_beta_uz <<endl;
                Gradient_vx += pow(dist-dist_1_,2) * 2*(vel.norm() - f_beta * vel_min_)*(vel(0) / vel.norm() * A_.row(3 * i + 1).transpose() - p_f_beta_ux * vel_min_) +
                               2*(dist-dist_1_)*dist_grad(0) * A_.row(3*i).transpose() *  pow(vel.norm() - f_beta * vel_min_,2);
                // cout<< "Gradient_vx \n"<< Gradient_vx <<endl;
                Gradient_vy += pow(dist-dist_1_,2) * 2*(vel.norm() - f_beta * vel_min_)*(vel(1) / vel.norm() * A_.row(3 * i + 1).transpose() - p_f_beta_uy * vel_min_) +
                               2*(dist-dist_1_)*dist_grad(1) * A_.row(3*i).transpose() *  pow(vel.norm() - f_beta * vel_min_,2);
                //cout << "Gradient_vy \n  "<< Gradient_vy <<endl;
                Gradient_vz += pow(dist-dist_1_,2) * 2*(vel.norm() - f_beta * vel_min_)*(vel(2) / vel.norm() * A_.row(3 * i + 1).transpose() - p_f_beta_uz * vel_min_)+
                               2*(dist-dist_1_)*dist_grad(2) * A_.row(3*i).transpose() *  pow(vel.norm() - f_beta * vel_min_,2);

                //cout <<"high_mpc_eva_end"<< endl;
            }
            cout <<"env adp end" << endl;
        }
