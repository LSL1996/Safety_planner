...
{   
    ...
    GridNodeMap = new GridNodePtr ** [GLX_SIZE];  // x
    for(int i = 0; i < GLX_SIZE; i++){
        GridNodeMap[i] = new GridNodePtr * [GLY_SIZE];  // y
        for(int j = 0; j < GLY_SIZE; j++){
            GridNodeMap[i][j] = new GridNodePtr [GLZ_SIZE];  // z
            for( int k = 0; k < GLZ_SIZE;k++){
...
void Astar::setParam(ros::NodeHandle& nh){
  ...
  nh.param("astar/if_safety_search", if_safety_search_, false);
  nh.param("astar/dist_safety", dist_safety_, -1.0);

  nh.param("astar/alpha1", alpha1_, -1.0);
  nh.param("astar/alpha2", alpha2_, -1.0);
  nh.param("astar/alpha3", alpha3_, -1.0);
...
bool Astar::isOccupied(const Eigen::Vector3i & index)
{
    ...
    double dist = edt_map_->sdf_map_->getDistance(pro_pos);
    if (dist <= margin_){
      return true;
    }else {
      return false;
...
void Astar::setEnvironment1(const EDTEnvironment::Ptr& env) {
    this->edt_map_ = env;
}


double Astar::getObsCost(const Eigen::Vector3d& cur_pt){
   double dist, fobs;
        ...
        */         
        for(int i = 0; i < (int)neighborPtrSets.size(); i++){
            ...
            */
            ...
            double cost;
            cost = getObsCost(neighborPtr -> coord);
            tentative_gScore = currentPtr -> gScore + alpha1_ * edge_cost + alpha2_ * cost ;
            double tentative_fScore = tentative_gScore + alpha3_ * getHeu(neighborPtr, endPtr);
            //cout << "currentPtr -> gScore                " << currentPtr -> gScore  << endl;
           // cout << "edge_cost                           " << edge_cost << endl;
            //cout << "getObsCost(neighborPtr -> coord)    " << 5 * getObsCost(neighborPtr -> coord)  << endl;
            if(! flag_explored){   //discover a new node
                ...
                */
                ...
                neighborPtr -> fScore    = neighborPtr -> gScore + alpha3_ * getHeu(neighborPtr, endPtr); //+ 10 * getObsCost(neighborPtr -> coord);
                neighborPtr -> nodeMapIt = openSet.insert(make_pair(neighborPtr->fScore, neighborPtr) ); //put neighbor in open set and record it.
                continue;
            }
            else if(tentative_fScore < neighborPtr-> fScore){ // in open set and need update
                /*
                *
                *
                ...
                */
                neighborPtr -> cameFrom = currentPtr;
                neighborPtr -> gScore = tentative_gScore ;
                neighborPtr -> fScore = tentative_gScore + alpha3_ * getHeu(neighborPtr, endPtr); //+ 10 * getObsCost(neighborPtr -> coord);
