...
void AdaptiveReplanFsm::init(ros::NodeHandle& nh){
    ...
    //waypoint_num_ = 1;
    //current_wp_ = 0;
    // waypoints_[0][0] = 14.5;  waypoints_[0][1] = -4.5;   waypoints_[0][2] = 1.0;
    ...
    if (flight_type_ == TARGET_TYPE::MANUAL_TARGET){
        ...
        trigger_sub_ = nh.subscribe("/sixinc/traj_start_trigger", 1, &AdaptiveReplanFsm::triggerCallback, this);
...
bool AdaptiveReplanFsm::callLowMpc(){
    ...
    bool plan_path_success = planner_manager_->LowMpc(start_pt_,start_vel_,end_pt_);
    if (plan_path_success){
        /* visulization */
