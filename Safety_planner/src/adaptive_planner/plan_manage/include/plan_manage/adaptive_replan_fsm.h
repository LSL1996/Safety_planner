...
private:
    ...
    std::vector<Eigen::Vector3d> history_pos1_, history_vel1_;
	...
	ros::Timer cmd_timer_, low_mpc_timer_, high_mpcc_timer_, safety_check_timer_, MLsearch_timer_;
	ros::Subscriber trigger_sub_, waypoint_sub_, odom_sub_, joy_sub_;

	ros::Publisher pos_cmd_pub_, pos_cmd_vis_pub_, exected_traj_pub_, end_pos_pub_, exected_traj1_pub_;
    ...
    void MLsearchCallback(const ros::TimerEvent& e);
	/* ROS functions */
    ...
    void draw_history_traj1();
    void planGlobalTrajbyGivenWps();

    static bool show_end_;
	/* helper functions */
	bool callLowMpc();
	bool callHighMpcc(Eigen::Matrix3d start_state);

    void draw_traj_our();
    std::vector<Eigen::Vector3d> our_pos_, our_vel_;
    void draw_traj_EVA();
    std::vector<Eigen::Vector3d> EVA_pos_, EVA_vel_;
    ros::Time  flag_start;
public:
	AdaptiveReplanFsm(){};
	~AdaptiveReplanFsm(){};
