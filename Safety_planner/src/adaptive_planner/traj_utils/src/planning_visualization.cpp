...
PlanningVisualization::PlanningVisualization(ros::NodeHandle& nh){
    ...
    ML_path_pub_       = node.advertise<visualization_msgs::Marker>("/planning_vis/ML_path", 20);                         // 6
    pubs_.push_back(ML_path_pub_);
}
...
void PlanningVisualization::drawMLsearchPath(const vector<Eigen::Vector3d>& path, double line_width,
                                     const Eigen::Vector4d& color, int id){
    displayLineStrip(path, line_width, color, 6);
}
